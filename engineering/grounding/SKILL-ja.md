...
確定したルール、口頭の実例、用語、決定、未解決の疑問は会話の文脈として後続作業へ引き継ぐ。これは追加ファイルの作成を許可する指示ではない。用語集と ADR 以外のファイルは、ユーザーから明示的に依頼された場合にのみ作成・更新する。仕様書と実装計画を求められたら、その情報を渡して `design-to-plan` を使う。
